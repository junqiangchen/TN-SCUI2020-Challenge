...
class ResNet2dModule(object):
    ...
    def train(self, train_images, train_lanbels, model_path, logs_path, learning_rate, dropout_conv=0.8, train_epochs=5,
        ...
        for i in range(train_epochs):
            ...
            train_images_data = train_images_data.astype(np.float)
            # Normalize from [0:255] => [0.0:1.0]
            batch_xs = np.multiply(train_images_data, 1.0 / 255.0)
    ...
    def prediction(self, test_images):
        ...
        predict_probvalue = np.zeros(test_images.shape[0], np.float)
