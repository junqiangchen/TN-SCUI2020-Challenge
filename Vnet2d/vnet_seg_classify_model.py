...
from Vnet2d.layer import (conv2d, deconv2d, upsample2d, normalizationlayer, crop_and_concat, resnet_Add,
                          weight_xavier_init, bias_variable, dense_to_one_hot)
...
def _next_batch(train_images, train_masks, train_labels, batch_size, index_in_epoch):
    ...
    return train_images[start:end], train_masks[start:end], train_labels[start:end], index_in_epoch


class Vnet2dmutiltaskModule(object):
    """
    ...
    def __init__(self, image_height, image_width, channels=1, n_class=2, inference=False, model_path=None,
                 costname=("dice coefficient", "cross_entropy")):
        ...
        self.Label_gt = tf.placeholder("float", shape=[None, self.n_class], name="Output_LabelGT")
        self.lr = tf.placeholder('float', name="Learning_rate")
        self.phase = tf.placeholder(tf.bool, name="Phase")
        self.drop_conv = tf.placeholder('float', name="DropOut")

        self.Y_pred, self.Label_pred_logits = _create_conv_net(self.X, self.image_width,
                                                               self.image_height,
                                                               self.channels,
                                                               self.phase,
                                                               self.drop_conv)
        self.Label_pred = tf.nn.softmax(self.Label_pred_logits)
        self.Label = tf.argmax(self.Label_pred, 1)

        self.segcost = self.__get_cost(costname[0], self.Y_gt, self.Y_pred)
        self.classifycost = self.__get_cost(costname[1], self.Label_gt, self.Label_pred_logits)

        self.cost = tf.reduce_mean(self.weight * self.segcost + self.classifycost)

        self.segaccuracy = self.__get_accuracy("dice coefficient", self.Y_gt, self.Y_pred)
        self.classifyaccuracy = self.__get_accuracy("classify accuracy", self.Label_gt, self.Label_pred)
    ...
    def __get_accuracy(self, accuracy_name, Y_gt, Y_pred, prob=0.8):
        """
        binary iou
        ...
        :return: binary iou
        """
        if accuracy_name == "dice coefficient":
            ...
            return metric
        if accuracy_name == "classify accuracy":
            correct_predict = tf.equal(tf.argmax(Y_pred, 1), tf.argmax(Y_gt, 1))
            accuracy = tf.reduce_mean(tf.cast(correct_predict, 'float'))
            return accuracy

    def train(self, train_images, train_mask, train_labels, model_name, logs_path, learning_rate,
        ...
        tf.summary.scalar("segaccuracy", self.segaccuracy)
        tf.summary.scalar("classifyaccuracy", self.classifyaccuracy)
        ...
        for i in range(train_epochs):
            # get new batch
            batch_xs_path, batch_ys_path, batch_label_path, index_in_epoch = _next_batch(train_images, train_mask,
                                                                                         train_labels, batch_size,
                                                                                         index_in_epoch)
            # label one_hot coding
            train_labels_onehot = dense_to_one_hot(batch_label_path, label_counts)
            batch_labels = train_labels_onehot.astype(np.float)
            ...
            batch_labels = batch_labels.astype(np.float)
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                train_loss, train_segaccuracy, train_classifyaccuracy = sess.run([self.cost, self.segaccuracy,
                                                                                  self.classifyaccuracy],
                                                                                 feed_dict={self.X: batch_xs,
                                                                                            self.Y_gt: batch_ys,
                                                                                            self.Label_gt: batch_labels,
                ...
                print('epochs %d training_loss ,Training_segaccuracy,Training_classifyaccuracy => %.5f,%.5f,%.5f' % (
                    i, train_loss, train_segaccuracy, train_classifyaccuracy))
    ...
    def prediction(self, test_images, prob=0.8):
        ...
        y_dummy = np.empty((1, self.n_class))
        segpred, label_prob, label = self.sess.run([self.Y_pred, self.Label_pred, self.Label],
                                                   feed_dict={self.X: test_images,
                                                              self.Y_gt: test_images,
                                                              self.Label_gt: y_dummy,
                                                              self.phase: 1,
                                                              self.drop_conv: 1})
        result = np.reshape(segpred, (test_images.shape[1], test_images.shape[2]))
        result[result >= prob] = 1.0
        result[result < prob] = 0.0
        result = result.astype(np.float32) * 255.
        result = np.clip(result, 0, 255).astype('uint8')
        label = np.clip(label, 0, 255).astype('uint8')
        return result, label
